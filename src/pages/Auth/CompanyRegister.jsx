...
const CompanyDetails = (props) => {
  ...
  // Validating input syntax using regex
  const checkValues = () => {
    let isValid = true;
  ...
  const checkInDB = () => {
    ...
    dataBase.companyList.companies.map((company, indexOfCompany) => {
      if (props.companyID === company.companyID) {
        toast.error("A company already exists with this id. Try another.");
        isValidInDB = false;
      }
    });
...
const AdminDetails = (props) => {
  ...
  const checkValues = () => {
    ...
    if (!adminName.test(props.adminNameC)) {
      ...
      toast.error(
        <div>
          ...
          <ul className="list-disc ml-4 mt-1">
            <li>Must start with a letter</li>
            <li>
              4 to 16 characters (lowercase letters, numbers, dots, underscores)
            </li>
      ...
      toast.error(
        "Password must be 4 to 16 characters (letters, numbers, or _-.@!#$).",
      );
      isValid = false;
    ...
    <>
      ...
      <div className="input-box-p text-lg bg-gray-100 rounded-2xl px-4">
        ...
        <div
          ...
          onClick={() => {
            props.data.passwordHidden
              ? props.data.setPasswordHidden(false)
              : props.data.setPasswordHidden(true);
          }}
        >
...
const CompanyRegister = (props) => {
  ...
  props.setSuccessOfRegistrationAsCompany(false);
